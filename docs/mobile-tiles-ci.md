...
Обновлённый APK установлен и открыт на Samsung SM-G780G 09.09.2026.
Владелец проверил новый кабинет: в настройках ключа видны только название
и создание секрета; поля домена/IP отсутствуют. Прежняя инструкция применить
домен в этом интерфейсе отозвана. Ограничения не настроены, §9 остаётся открытым.
Не создавать новый секрет и не просить владельца повторно искать поля.
...
Текст для будущего уточнения у поддержки (не отправлен):
«Для бесплатного Tiles API в новом maps-api/console в настройках ключа доступны
только название и создание секрета. Поддерживаются ли ограничения по Referer
для этого продукта и тарифа, где их включить и применяются ли они к бесплатным
запросам тайлов? Клиент — Flutter Android/iOS, Referer https://sosedi-app.ru/.»
Значение ключа для такого вопроса не требуется.
