...
Фактический полный CI 29.07.2026 покрывает 59,10% строк backend и 80,70% строк
Flutter без generated-кода. Backend denominator исключает только
декларативные Nest modules/controllers/DTO/decorators и `main.ts`; guards,
services, operations и business/security logic остаются в gate. Пороги нельзя
добивать тестами простой вёрстки, но они не позволяют вернуть покрытие логики к
исходному уровню.
...
Воспроизводимые без production credentials отказы закреплены автоматическими
проверками:

- SMS.ru имеет timeout 5 секунд, безопасную ошибку и успешный повтор после
  восстановления;
- readiness возвращает безопасный `503` при отказе Redis, а logout не скрывает
  незавершённый отзыв сессии;
- ошибка S3 не удаляет связь upload с БД и не маскируется под отсутствующий файл;
- исключение push-провайдера переводит delivery в bounded backoff, после
  восстановления событие доставляется один раз и больше не выбирается;
- fake payment provider и мобильные команды сохраняют idempotency/request ID при
  неоднозначном ответе;
- mobile различает offline/timeout, сохраняет уже загруженный каталог и повторяет
  создание объявления/загрузку фото без второго Item.

Live-проверки SMS/S3/push и остальных provider credentials остаются отдельной
частью staging/production smoke: unit-тесты не считаются доказательством
