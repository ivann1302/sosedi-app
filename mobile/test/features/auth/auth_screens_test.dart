...
void main() {
  ...
  testWidgets('keeps onboarding usable on a compact screen at 200% text', (
    ...
    await _pumpCompactScreen(tester, controller, const OnboardingScreen());
    await tester.pumpAndSettle();

    for (var index = 0; index < 3; index += 1) {
      expect(tester.takeException(), isNull);
      expect(find.byKey(ValueKey('onboarding-image-$index')), findsOneWidget);
      final buttonLabel = index == 2 ? 'Смотреть вещи' : 'Далее';
      expect(
        find.widgetWithText(FilledButton, buttonLabel).hitTestable(),
        findsOneWidget,
      );
      if (index < 2) {
