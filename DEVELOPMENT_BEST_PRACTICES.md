...
- Интеграции с S3, push-транспортами, CloudPayments и другими внешними системами
...
- Только для `LOCAL_KYC` документы находятся в отдельном private bucket/prefix с
  отдельными правами и аудитом; при `PROVIDER_MANAGED` «Всё рядом» их не собирает.
...
## 10. Бронирования и CloudPayments
...
> monetized MVP разрешён только через согласованную «Безопасную сделку»
> CloudPayments;
> оплата при передаче остаётся pilot, где «Всё рядом» не принимает деньги и берёт 0%
...
- В online-сценарии webhook считается недоверенным вводом: проверить CloudPayments
  HMAC по исходному телу, сверить transaction/deal/environment и при
  неоднозначности запросить состояние по `EscrowAccumulationId`.
...
Рекомендации адаптированы под ограничения «Всё рядом», а не скопированы как универсальная
архитектура:
