...
| 2 | Подключает имеющиеся Android и iPhone к Mac, разблокирует и подтверждает trust/debug | 5–10 мин |
| 3 | Передаёт путь к исходникам промосайта либо готовит SprintHost export | 10–30 мин |
...
| 7 | После Store readiness создаёт MapKit key и проходит MapKit device smoke | 1–2 ч |
...
### Следующее минимальное действие — только после автономной подготовки

Codex сначала исправляет debug-доступ Android к локальному backend, пересобирает
APK и повторяет доступные проверки. После этого владельцу нужно только:

1. Подключить Android и iPhone к Mac по USB.
2. Разблокировать оба телефона.
3. На Android подтвердить USB debugging/RSA, на iPhone — «Доверять этому
   компьютеру» и Developer Mode, если iOS его запросит.

Аккаунты магазинов, ключ MapKit, SMS/S3 и юридические документы для этого шага
...
- Домен `sosedi-app.ru` уже занят отдельным промолендингом на SprintHost. Public
  legal/support paths планируются на том же домене; перенос в Timeweb — только
  через technical preview и rollback. Исходники Next.js landing/export пока не
  переданы.
- Public support contact: `sosedi.rs@yandex.ru`.
- Timeweb Cloud account есть, но VPS, OCI registry и S3-ресурсы не созданы.
- Физические Android и iPhone есть; к текущему Mac-сеансу пока не подключены.
  Наличие отдельного Android без GMS не подтверждено.
...
Для ближайшего local smoke уже подтверждено наличие одного Android и одного
iPhone. Это отдельный ранний шаг без магазинов и push: достаточно USB trust/debug
и одной сети с Mac. Он проверяет установку, onboarding, console OTP, каталог и
основную навигацию, но не закрывает release-пункты ниже.

- [ ] Подключить имеющиеся Android и iPhone к Mac для local debug smoke.
