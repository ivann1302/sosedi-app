...
| 12 | Организует районы, предложение, поддержку и пять usability-респондентов | недели |
...
### Результат инвентаризации — 06.09.2026

- Apple Developer, Google Play Console и RuStore Console не созданы; ждём юрлицо
  заказчика и не открываем временные личные store-аккаунты.
- Домен `sosedi-app.ru` используется промолендингом на SprintHost. Решением
  владельца 09.09.2026 перенос в Timeweb отменён; public legal/support pages
  размещаем на текущем хостинге и домене. Исходники получены:
...
Первые пять route уже существуют в локальном Astro `public-web`. Решением
владельца 09.09.2026 перенос public site в Timeweb отменён. Сайт остаётся на
SprintHost и домене `sosedi-app.ru`; API-токен Timeweb для этой задачи не нужен.
Владелец передал `/Users/ivan/projects/sosedi` и подтвердил актуальность. В
исходниках есть `/` и `/business`; старое описание `/blog` и `/contacts` не
подтверждено исходниками. Mirrored HTML не становится новым source of truth.

Локальная подготовка 09.09.2026 завершена: объединённое preview, ссылки и
раздельные security headers проверены. Caddy используется только локально;
способ размещения общей сборки и headers на SprintHost ещё нужно проверить.
Детали и команды:
[`docs/public-site-preview-2026-09-09.md`](docs/public-site-preview-2026-09-09.md).

Минимальная схема обновления на SprintHost:

**Ответственный — владелец (09.09.2026):** публикацию выполняет самостоятельно.
Codex напоминает об этом в итогах работы; до подтверждения публикации и HTTPS
smoke пункты §7.3 остаются открытыми. Не запрашивать доступ к SprintHost для
самостоятельного deploy Codex.

1. Проверить текущий способ запуска лендинга, document root и доступные
   настройки HTTP headers по конфигурации SprintHost.
2. Подготовить размещение Astro pages и assets рядом с лендингом; проверить
   раздельные headers и все routes в тестовом размещении на текущем хостинге.
3. Сохранить резервную копию текущих файлов и конфигурации для rollback.
4. После проверки опубликовать подготовленное обновление на `sosedi-app.ru`,
   проверить HTTPS, routes/assets, support email и отсутствие forms/analytics
   на legal pages. Черновики документов не считать утверждёнными.
5. Сохранить инструкции обновления и возврата к предыдущей сборке. DNS и
   провайдера хостинга не менять.
