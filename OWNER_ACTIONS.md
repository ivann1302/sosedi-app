...
- Старый лендинг «Соседи» на `sosedi-app.ru` и SprintHost остаётся без
  изменений. Для «Всё рядом» создаём отдельный новый лендинг на Timeweb и новом
  домене. Счёт Timeweb выставлен, ожидается оплата. Исходники старого сайта
  сохранены в `/Users/ivan/projects/sosedi` и не являются основой нового сайта.
...
Route-контракт нового сайта после выбора домена:

```text
https://NEW_DOMAIN/                                  новый лендинг «Всё рядом»
https://NEW_DOMAIN/documents/                       список документов
https://NEW_DOMAIN/documents/<slug>/<version>/      versioned документ
https://NEW_DOMAIN/support/                         публичная поддержка
https://NEW_DOMAIN/account-deletion/                закрытие аккаунта
```

`NEW_DOMAIN` здесь обозначает ещё не выбранное имя, а не значение для конфига.
Точные API/operator hostnames фиксируются после покупки домена.

Первые пять route уже существуют в локальном Astro `public-web`. Решением
владельца 15.09.2026 старый лендинг «Соседи» остаётся на SprintHost и
`sosedi-app.ru` без изменений. Для «Всё рядом» создаётся отдельный новый landing
на Timeweb и новом домене; счёт выставлен, ожидается оплата, домен ещё не выбран.
Исходники `/Users/ivan/projects/sosedi` относятся только к старому сайту и не
переносятся автоматически в новый проект.

Локальная подготовка Astro routes 09.09.2026 завершена: preview, ссылки и
раздельные security headers проверены. Эти страницы нужно встроить в новый сайт
«Всё рядом» после выбора домена и оплаты Timeweb. Детали прежнего preview:
[`docs/public-site-preview-2026-09-09.md`](docs/public-site-preview-2026-09-09.md).

Минимальная схема нового лендинга на Timeweb:

**Текущий blocker 15.09.2026:** ожидаются оплата счёта Timeweb и выбор нового
домена. Старый SprintHost не является target для deploy и не требует доступа.

1. Закрыть видимое переименование проекта в «Всё рядом» по §7.0.
2. После оплаты создать новый статический landing и подключить Astro
   legal/support/account-deletion routes.
3. Привязать новый домен, включить HTTPS и security headers.
4. Проверить responsive/accessibility, все публичные routes и отсутствие
   forms/analytics на legal pages; черновики документов не считать утверждёнными.
5. Сохранить инструкции deploy/rollback и только после smoke занести точные URL
   в mobile и store consoles.
...
выполняются после технического запуска всего приложения. Старый SprintHost-сайт
не меняется; новый Timeweb-лендинг ведётся отдельным пунктом §7.3
`MVP_CHECKLIST.md` и нужен до store listing/public release.
