...
- [ ] письменное подтверждение CloudPayments о доступности «Безопасной сделки»
  ...
  **WAITING 15.09.2026:** первичная заявка отправлена; ожидаются письменный
  ответ, коммерческие условия и связанные test-терминалы оплаты/выплат.
...
Проверка SprintHost отложена до технического запуска всего приложения, как
и определение районов и организация поддержки: `MVP_CHECKLIST.md`, приложение A.
Сейчас Codex не запрашивает эти действия. Обязательные публичные URL из §7.3
остаются условием открытия публичной аренды. Не запрашивать доступ к SprintHost для
...
**Порядок от 09.09.2026:** определение районов пилота, назначение поддержки
и проверка SprintHost выполняются после технического запуска всего приложения.
Их канонические задачи перенесены в приложение A `MVP_CHECKLIST.md`;
приведённые здесь инструкции сохранены для этого этапа.
